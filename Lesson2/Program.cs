...
//     //await context.Response.WriteAsync(File.ReadAllText($"html/index.html"));
// });
...
app.MapPost("/api/users/login", handler: (User user) =>
{
    using(UserContext db = new UserContext()) {
        ...
        else return Results.BadRequest("User not Found");
    }

});

app.MapPost("/api/user/register", handler: (User user) => {
    using(UserContext db = new UserContext()) {
        var finduser = db.Users.SingleOrDefault(u => u.Login == user.Login);
        if(finduser != null) {
...
app.MapDelete("/api/user{ID:long}", handler: (User user)=>{
    using(UserContext db = new UserContext()) {
        var finduser = db.Users.SingleOrDefault(u => u.Login == user.Login);
        ...
        else return Results.BadRequest("Wrong password");
    }
});
